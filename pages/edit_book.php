...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titleErr) && empty($authorErr) && empty($descriptionErr) && empty($priceErr) && empty($categoryErr)) {
        $isUniqueBookTitle = isUniqueBookTitleNotItself($title, $_GET['book_id']);
        if ($isUniqueBookTitle === false) {
            $titleErr = 'Book title already exists';
        } else {
            $result = updateBook($title, $author, $description, $price, $category, $_GET['book_id']);
            if ($result === 'success') {
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="category" name="category">
                        ...
                        <?php
                            ...
                            ?>
                                <option value="<?php echo $category['id']?>" <?php if($category['id'] === $book['category_id']) echo 'selected' ?>> <?php echo $category['name'] ?> </option>;                     
                        <?php
                            }
                        ?>
                    </select>
